...
{
    public partial class CountUp : UserControl
    {
        ...
        DateTime SetTime; // the time to shutdown
        ...
        powerDel runPower;
        CountTimerControl CountUpCountTimerControl;
        ...
        private void btnSetTime_Click(object sender, EventArgs e)
        {
            if (isSetValid())
            {
                ...
                {
                    SetTime = getSetTime();
                    lbShuTime.Text = SetTime.ToString(timeFormat);
                    lbShuTT.Text = SetTime.ToString("tt");
        ...
        private bool Alert()
        {
            ...
            am = new AlertMessage(CountUpCountTimerControl);
        ...
        public bool Start() // handles the start operation
        {
            if (lbStatus.Text != "started")
            {
                ...
                if (currCounter == "null" || (lbStatus.Text == "paused" && currCounter == "cu"))
                {
                    ...
                    try
                    {
                        ...
                        Properties.Settings.Default.currentCounter = "cu";
                        return true;
                    }
                    catch
                    {
                ...
                else
                {
                    if (currCounter == "cd")
                    {
                        notification.Show("Count Down timer has already been started");
                    }
                    else if (currCounter == "cu")
                    {
        ...
        private void tmCurTimer_Tick(object sender, EventArgs e)
        {
            ...
            int hrs = (ts.Hours > -1 && ts.Minutes > -1 && ts.Seconds > -1) ? ts.Hours : (ts.Hours + 23);
            int min = (ts.Minutes > -1) ? ts.Minutes : (ts.Minutes + 60);
            int sec = (ts.Seconds > -1) ? ts.Seconds : (ts.Seconds + 60);
        ...
        private void setCircleProgress()
        {
            if(!isComplete && isStart)
            {
                try
                {
                    ...
                    TimeSpan ts = SetTime - DateTime.Now;
                    TimeSpan ts2 = SetTime - timeStarted;

                    double secsRem = Convert.ToInt32(ts.TotalSeconds);
                    secsRem = (secsRem > -1) ? secsRem : (secsRem + 86400);
                    double secsSet = Convert.ToInt32(ts2.TotalSeconds);
